...
public class VpTokenValidator {
    ...
    @Value("${dca.verifier.origin:https://verifier.localhost:8443}")
    private String origin;

    public VpUserEntity validate(Object vpToken, String nonce) {
        if (vpToken instanceof String token) {
            try {
                ...
                MDocVerificationParams verificationParams = getVerificationParams(nonce);
                SimpleCOSECryptoProvider validationCryptoProvider = new SimpleCOSECryptoProvider(List.of(getIssuerKeyInfo(issuerCertificateChain, issuerTrustedRootCAs), getDeviceKeyInfo(mDoc)));
    ...
    private MDocVerificationParams getVerificationParams(String nonce) {
        return new MDocVerificationParams(
                VerificationType.Companion.getAll(),
                KEY_ID_ISSUER,
                KEY_ID_DEVICE,
                null,
                getDeviceAuthentication(nonce),
                null
        );
    }

    @SneakyThrows
    public DeviceAuthentication getDeviceAuthentication(String nonce) {
        ListElement handover = new ListElement(List.of(new StringElement(origin), new StringElement(nonce), new NullElement()));
